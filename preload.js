...
contextBridge.exposeInMainWorld('electron', {
  ...
  listAircraft: (installPath) => ipcRenderer.invoke('list-aircraft', installPath),
  ...
  downloadsSave: (relPath, buffer, encrypt = false) => {
    const u8 = buffer instanceof Uint8Array ? buffer : new Uint8Array(buffer || []);
    return ipcRenderer.invoke('downloads:save', { relPath, buffer: u8, encrypt });
  },

  // Events
  onInstallProgress: (handler) => {
    // Forward payloads verbatim so renderer can use {percent, progress, status, phase} or plain numbers
    const listener = (_e, payload) => { try { handler(payload); } catch {} };
    ipcRenderer.on('install-progress', listener);
    return () => ipcRenderer.removeListener('install-progress', listener);
  },

  // Paths
  selectFolder: (defaultPath) => ipcRenderer.invoke('dialog:openDirectory', defaultPath),
  getDefaultInstallPath: () => ipcRenderer.invoke('get-default-install-path'),
  getDefaultInstallPath2024: () => ipcRenderer.invoke('get-default-install-path-2024'),
  // Persisted install paths in main settings (robust across localStorage/file:// origin changes)
  getSavedInstallPath2020: () => ipcRenderer.invoke('settings:get-install-path-2020'),
  getSavedInstallPath2024: () => ipcRenderer.invoke('settings:get-install-path-2024'),
  setSavedInstallPath2020: (dirPath) => ipcRenderer.invoke('settings:set-install-path-2020', dirPath),
  setSavedInstallPath2024: (dirPath) => ipcRenderer.invoke('settings:set-install-path-2024', dirPath),
  ...
  downloadsFetchUrl: (relPath, url, encrypt = false, context = '') => ipcRenderer.invoke('downloads:fetch-url', { url, relPath, encryptAtRest: encrypt, context }),
  ...
  readChangelogFromZip: (absZipPath) => ipcRenderer.invoke('downloads:read-changelog-from-zip', absZipPath),
  ...
  openFolder: (absPath) =>
    ipcRenderer.invoke('os:open-folder', absPath),

  // Secure token storage
  getSavedToken: (account = 'default') => ipcRenderer.invoke('auth:get-token', account),
  saveToken: (token, account = 'default') => ipcRenderer.invoke('auth:set-token', { account, token }),
  clearToken: (account = 'default') => ipcRenderer.invoke('auth:clear-token', account),

  // Downloads directory get/set
  getDownloadsDir: () => ipcRenderer.invoke('downloads:get-dir'),
  setDownloadsDir: (dirPath) => ipcRenderer.invoke('downloads:set-dir', dirPath),
  ...
  findExtractDirForFolder: (folder) => ipcRenderer.invoke('pkg-cache:find-extract-dir', { folder }),

  // Network fetch via main process (bypasses renderer CORS). Returns { ok, status, headers, text }.
  netFetchText: (url, options = {}) => ipcRenderer.invoke('net:fetch-text', { url, ...(options || {}) }),
  // Network HEAD/meta via main (returns { ok, status, etag, lastModified, contentLength })
  netHead: (url, options = {}) => ipcRenderer.invoke('net:head', { url, ...(options || {}) }),
