  ...
  try {
    win.webContents.on(
      ...
      (_event, level, message, line, sourceId) => {
        try {
          const lvl =
            { 0: "LOG", 1: "WARN", 2: "ERROR" }[level] || String(level);
...
ipcMain.handle("downloads:read-changelog-from-zip", async (_e, absZipPath) => {
  try {
    if (!absZipPath || typeof absZipPath !== "string")
      throw new Error("Missing zip path");
    let zipPath = absZipPath;
    let tempToDelete = "";
    try {
      const hdr = await fs.promises
        .readFile(zipPath, { encoding: null })
        .then((b) => b.slice(0, 4))
        .catch(() => null);
      if (
        /\.enc$/i.test(zipPath) ||
        (hdr && Buffer.isBuffer(hdr) && hdr.length >= 4 && hdr[0] !== 0x50)
      ) {
    ...
    for (const e of entries) {
      if (!e.isDirectory && isCandidate(e.entryName)) {
        chosen = e;
        break;
      }
    }
    if (!chosen) {
      ...
      const kw =
        /(change|update|release|what\s*'s\s*new|whatsnew|notes|history|readme)/i;
      for (const e of entries) {
        if (
          !e.isDirectory &&
          /\.(txt|md)$/i.test(e.entryName) &&
          kw.test(e.entryName)
        ) {
          chosen = e;
          break;
    ...
    if (!chosen) {
      ...
      try {
        ...
        if (manifestEntry) {
          ...
          const lu = normField(
            mJson.LastUpdate || mJson.lastUpdate || mJson.last_update || "",
          );
          const rn = normField(
            mJson.ReleaseNotes ||
              mJson.releaseNotes ||
          ...
          if (parts.length) {
            if (tempToDelete) {
              try {
                await fs.promises.rm(tempToDelete, { force: true });
              } catch {}
            }
            return {
              success: true,
    ...
    try {
      ...
      if (manifestEntry) {
        ...
        const rn = normField(
          mJson.ReleaseNotes || mJson.releaseNotes || mJson.release_notes || "",
        );
        const oh = normField(
          mJson.OlderHistory ||
            mJson.olderHistory ||
...
app.whenReady().then(() => {
  ...
  if (autoUpdater) {
    try {
      ...
      autoUpdater.logger = {
        info: (...a) => console.log("[updater:info]", ...a),
        warn: (...a) => console.warn("[updater:warn]", ...a),
        error: (...a) => console.error("[updater:error]", ...a),
      };
...
async function detectFS2020() {
  ...
  for (const p of candidates) {
    try {
      ...
      const c = await readInstalledPackagesPathFrom(p);
      if (c) return c;
    } catch {}
  }
...
async function detectFS2024() {
  ...
  const candidates = [
    ...
    path.join(
      L,
      "Packages",
      "Microsoft.MicrosoftFlightSimulator_8wekyb3d8bbwe",
      "LocalCache",
      "UserCfg.opt",
    ),
    path.join(
...
async function downloadToFile(
  url,
  destFile,
  { maxRedirects = 5, timeoutMs = 300000 } = {},
) {
  ...
  const doReq = (u, redirectsLeft) =>
    new Promise((resolve, reject) => {
      ...
      const req = lib.get(
        u,
        { headers: { "User-Agent": "SWS-Installer/1.0" } },
        (res) => {
          if (
            res.statusCode >= 300 &&
            res.statusCode < 400 &&
...
async function headRequest(url, redirectsLeft = 5) {
  return new Promise((resolve, reject) => {
    ...
    const req = lib.request(
      url,
      { method: "HEAD", headers: { "User-Agent": "SWS-Installer/1.0" } },
      (res) => {
...
async function rangeMetaRequest(url, redirectsLeft = 5) {
  return new Promise((resolve, reject) => {
    const lib = url.startsWith("https:") ? https : http;
    const opts = {
      headers: { "User-Agent": "SWS-Installer/1.0", Range: "bytes=0-0" },
    };
    const req = lib.get(url, opts, (res) => {
      ...
      if (!total)
        total =
          Number(headers["content-length"] || headers["Content-Length"] || 0) ||
          0;
      const info = {
        statusCode: status,
        contentLength: total,
...
async function downloadToDownloadsWithResume(
  event,
  { url, relPath, encryptAtRest = false, context = "" },
) {
  const sender = event?.sender;
  const base = getDownloadsBaseDir();
  const finalPath = path.resolve(base, relPath);
  debugLog("INFO", "Download started", {
  ...
  if (serverContentChanged) {
    ...
    debugLog(
      "INFO",
      "Download: server content changed, invalidating cached zip",
      { file: path.basename(relPath) },
    );
  }
  ...
  function sendProgress(pct, recBytes = null) {
    try {
      ...
      const r =
        Number.isFinite(recBytes) && recBytes >= 0 ? recBytes : startSize;
      if (sender)
        sender.send("download-progress", { context, pct, received: r, total });
      else
        sendInstallProgress({
      ...
      if (pct >= lastLoggedPct + 10 || now - lastLogTs > 15000 || pct === 100) {
        ...
        try {
          console.log(
            "[DOWNLOAD:progress] " +
              JSON.stringify({ context, pct, received: r, total }),
          );
        } catch {}
      }
    } catch {}
  ...
  try {
    ...
    if (
      stFinal.size > 0 &&
      (!total || stFinal.size === total || encryptAtRest)
    ) {
  ...
  await new Promise((resolve, reject) => {
    ...
    const doGet = (u, redirectsLeft = 5) => {
      const req = lib.get(u, { headers: makeHeaders() }, (res) => {
        ...
        ws.on("error", (err) => {
          try {
            console.error("[DOWNLOAD:ws-error]", err?.message || err);
          } catch {}
          debugLog("ERROR", "Download write-stream error", {
            file: path.basename(relPath),
        ...
        const stallTimer = setInterval(() => {
          if (Date.now() - lastDataTs > 60000) {
            try {
              console.error(
                "[DOWNLOAD:stall] No data received for 60s, aborting",
              );
            } catch {}
            debugLog("ERROR", "Download stalled — no data for 60s", {
              file: path.basename(relPath),
              received,
              total,
            });
            ...
            try {
              req.destroy(
                new Error("Download stalled — no data for 60 seconds"),
              );
            } catch {}
          }
        }, 10000);
      ...
      currentReq = req;
      activeDownloadMap.set(context, {
        abort: () => {
          destroyed = true;
          try {
            currentReq && currentReq.destroy(new Error("Canceled"));
          } catch {}
          try {
            currentWs && currentWs.destroy();
          } catch {}
        },
      });
      req.on("error", (err) => {
  ...
  try {
    console.log(
      "[DOWNLOAD:complete] " + JSON.stringify({ relPath, finalPath }),
    );
  } catch {}
...
async function fetchTextWithRedirects(
  ...
  return new Promise((resolve, reject) => {
    ...
    const doRequest = (u, redirectsLeft) => {
      ...
      try {
        const key = new URL(u).toString();
        if (seen.has(key)) return reject(new Error("Redirect loop"));
        seen.add(key);
      } catch {}
...
async function decryptFileToTemp(absEncPath) {
  ...
  const tmpZip = path.join(
    tmpDir,
    path.basename(absEncPath).replace(/\.enc$/i, "") || `tmp-${Date.now()}.zip`,
  );
...
ipcMain.handle("settings:get-install-path-2020", async () => {
  try {
    return String(readSettings().installPath2020 || "").trim();
  } catch {
    return "";
  }
});
ipcMain.handle("settings:get-install-path-2024", async () => {
  try {
    return String(readSettings().installPath2024 || "").trim();
  } catch {
    return "";
  }
});
...
ipcMain.handle("settings:set-debug-logging", async (_e, enabled) => {
  try {
    ...
    debugLog(
      "INFO",
      val ? "Debug logging enabled by user" : "Debug logging disabled by user",
    );
...
ipcMain.handle(
  "downloads:fetch-url",
  async (event, { url, relPath, encryptAtRest = false, context = "" }) => {
    try {
      const res = await downloadToDownloadsWithResume(event, {
        url,
        relPath,
        encryptAtRest,
        context,
...
ipcMain.handle("downloads:cancel", async (_e, context) => {
  try {
    activeDownloadMap.get(context)?.abort();
    return { success: true };
  } catch (e) {
    return { success: false, error: e?.message || String(e) };
  }
...
ipcMain.handle(
  "auth:set-token",
  async (_e, { account = "default", token = "" } = {}) => {
    try {
...
ipcMain.handle("dialog:openDirectory", async (_e, defaultPath) => {
  ...
  if (defaultPath) {
    ...
    try {
      const st = await fs.promises.stat(dp);
      if (st.isDirectory()) opts.defaultPath = dp;
    } catch {}
  }
...
ipcMain.handle(
  "save-file",
  async (_e, { relPath, buffer, encrypt = false }) => {
    try {
      return await saveFileImpl({ relPath, buffer, encrypt });
    } catch (err) {
...
ipcMain.handle(
  ...
  async (_e, { relPath, buffer, encrypt = false }) => {
    try {
      return await saveDownloadImpl({ relPath, buffer, encrypt });
    } catch (err) {
      return { error: err.message || String(err) };
    }
  },
...
ipcMain.handle(
  "net:fetch-text",
  async (_e, { url, timeoutMs = 15000, method, headers, body } = {}) => {
    try {
      ...
      try {
        for (const [k, v] of Object.entries(res.headers || {})) {
          headersNorm[String(k).toLowerCase()] = Array.isArray(v)
            ? v.join(", ")
            : String(v || "");
        }
      } catch {}
    ...
    const results = await Promise.all(
      dirNames.map(async (folder) => {
        ...
        try {
          ...
          const realPkgDir = await fs.promises
            .realpath(pkgDir)
            .catch(() => pkgDir);
        ...
        if (!version) {
          try {
            version = await readPackageManifestVersion(pkgDir);
          } catch {}
        }
        return {
          folder,
...
ipcMain.handle("uninstall-aircraft", async (_e, opts = {}) => {
  try {
    debugLog("INFO", "Uninstall started", {
    ...
    if (name) {
      ...
      const hit =
        candidates.find((x) => x.s.includes(key)) ||
...
ipcMain.handle("install-aircraft", async (_e, opts = {}) => {
  try {
    ...
    try {
      ...
      throw new Error(
        `Install path is not writable: ${dest}. Please choose your Community folder or check permissions. (${e?.message || e})`,
      );
    }
    ...
    const simTag = /^FS2024$/i.test(String(opts.simTag || ""))
      ? "FS2024"
      : /^FS2020$/i.test(String(opts.simTag || ""))
        ? "FS2020"
        : "FS";
    const channel = /^beta$/i.test(String(opts.channel || ""))
      ? "Beta"
      : "Public";
    ...
    try {
      ...
      if (st && st.isDirectory()) {
        ...
        const zipDiskTime = Math.max(
          zipStat?.birthtimeMs || 0,
          zipStat?.mtimeMs || 0,
        );
        ...
        try {
          ...
          const savedSize = await fs.promises
            .readFile(sizeFile, "utf8")
            .catch(() => "");
          if (
            savedSize &&
            zipStat &&
            String(zipStat.size) !== savedSize.trim()
          ) {
            zipSizeChanged = true;
          }
        } catch {}
        if (zipNewerThanCache || zipSizeChanged) {
          ...
          const ents = await fs.promises
            .readdir(extractDir, { withFileTypes: true })
            .catch(() => []);
          for (const d of ents) {
            if (!d.isDirectory()) continue;
            const pkg1 = path.join(extractDir, d.name, "manifest.json");
            const ok1 = await fs.promises
              .stat(pkg1)
              .then(() => true)
              .catch(() => false);
            if (ok1) {
            ...
            const innerEnts = await fs.promises
              .readdir(path.join(extractDir, d.name), { withFileTypes: true })
              .catch(() => []);
            const onlyDir = innerEnts.filter((e) => e.isDirectory());
            if (onlyDir.length === 1) {
              const pkg2 = path.join(
                extractDir,
                d.name,
                onlyDir[0].name,
                "manifest.json",
              );
              const ok2 = await fs.promises
                .stat(pkg2)
                .then(() => true)
                .catch(() => false);
              if (ok2) {
    ...
    if (!reuseExtract) {
      ...
      try {
        if (tempToDelete) await fs.promises.rm(tempToDelete, { force: true });
      } catch {}
      // Save ZIP file size so future installs can detect content changes even when mtime is unreliable
      try {
        const _zs = await fs.promises.stat(zip).catch(() => null);
        if (_zs)
          await fs.promises.writeFile(
            path.join(extractDir, ".sws-zip-size"),
            String(_zs.size),
          );
      } catch {}
    } else {
    ...
    try {
      if (!preferCopy) {
        ...
        if (maxPathLen > 240) {
          try {
            sendInstallProgress({
              status: `Note: long file paths detected (max ${maxPathLen}). Junction will still be attempted.`,
              phase: "extract",
            });
          } catch {}
        }
    ...
    try {
      const expectedRaw = Array.isArray(opts.expectedFolders)
        ? opts.expectedFolders
        : [];
      const norm = (s) =>
        String(s || "")
          .trim()
      ...
      if (expected.length) {
        ...
        if (filtered.length) {
          ...
          topDirs = filtered.sort(
            (a, b) => scoreMatch(b) - scoreMatch(a) || a.localeCompare(b),
          );
        }
      }
    } catch {}
    if (!topDirs.length) {
      ...
      const moveEntries = await fs.promises.readdir(extractDir, {
        withFileTypes: true,
      });
    ...
    for (const dir of topDirs) {
      ...
      try {
        const hasManifestAt = async (p) =>
          await fs.promises
            .stat(path.join(p, "manifest.json"))
            .then(() => true)
            .catch(() => false);
        const hasLayoutAt = async (p) =>
          await fs.promises
            .stat(path.join(p, "layout.json"))
            .then(() => true)
            .catch(() => false);
        let current = target;
      ...
      try {
        const hasManifestFinal = await fs.promises
        ...
        if (!hasManifestFinal) {
          ...
          try {
            sendInstallProgress({
              status: `Skipping '${linkName}' (no manifest.json)`,
              phase: "link",
            });
          } catch {}
          continue;
      ...
      try {
        ...
        try {
          sendInstallProgress({
            status: `Warning: could not remove existing '${linkName}': ${rmErr?.message}`,
            phase: "link",
          });
        } catch {}
      }
      ...
      try {
        const anyOneDrive =
          /onedrive/i.test(String(target)) || /onedrive/i.test(String(dest));
        if (anyOneDrive) {
          try {
            sendInstallProgress({
              status: `Note: OneDrive path detected for '${linkName}'. Junction will still be attempted.`,
              phase: "link",
            });
          } catch {}
        }
      ...
      if (preferCopy) {
        ...
        await fs.promises.cp(target, linkPath, {
          recursive: true,
          force: true,
        });
      } else {
        ...
        try {
          ...
          debugLog("INFO", "Junction created successfully", {
            folder: linkName,
          });
        } catch (e) {
          linkFallbackUsed = true;
          debugLog("WARN", "Junction failed, falling back to copy", {
            folder: linkName,
            error: e?.message || String(e),
          });
          try {
            sendInstallProgress({
              status: `Symlink failed for '${linkName}': ${e?.message || e}. Copying instead.`,
              phase: "link",
            });
          } catch {}
          await fs.promises.cp(target, linkPath, {
      ...
      const stepPct = Math.round(
        ((idx + 1) / Math.max(1, topDirs.length)) * linkRange,
      );
    ...
    if (installedFolders.length === 0) {
      const listed =
        topDirs && topDirs.length ? ` Examined: ${topDirs.join(", ")}.` : "";
      throw new Error(
        `Install failed after extraction: no MSFS package (manifest.json) was linked or copied into Community.${listed} Check that the ZIP contains a proper package and that the install path is writable.`,
      );
    }
    ...
    try {
      ...
      const names = Array.isArray(installedFolders)
        ? installedFolders.slice()
        : [];
      ...
      const stripAffixes = (n) =>
        String(n || "")
          .replace(/^z{1,3}[-_]/i, "")
          .replace(/[-_](pms|pmsgtn|pms50|tds|gtn|gtn750|mod|panel)$/i, "")
          .replace(/[-_](beta|public)$/i, "")
          .trim();
      const baseProjections = Array.from(
        new Set(names.map(stripAffixes)),
      ).filter(Boolean);
      ...
      if (version) {
        ...
        function scoreName(n) {
          ...
          let score = 0;
          if (preferredBaseToken && s.includes(preferredBaseToken))
            score += 200; // hard preference for the aircraft base
    ...
    // Done
    debugLog("INFO", "Install completed", {
      installedFolders,
      version,
      linkFallbackUsed,
      communityPath: dest,
...
ipcMain.handle(
  "get-package-lastupdate",
  async (_e, { installPath, folder }) => {
    try {
      if (!installPath || !folder) throw new Error("Missing params");
      const pkgDir = path.join(String(installPath), String(folder));
      // Resolve junction/symlink target so we read from the real cache directory
      ...
      const hasAnyText = (obj, depth = 0) => {
        ...
        for (const val of Object.values(obj)) {
          ...
          if (
            Array.isArray(val) &&
            val.some((x) => typeof x === "string" && String(x).trim())
          )
            return true;
          if (val && typeof val === "object" && hasAnyText(val, depth + 1))
            return true;
        }
        return false;
      };
        ...
        if (!olderHistory) {
          for (const lb of langBuckets) {
            if (j[lb] && typeof j[lb] === "object") {
              for (const k of olderKeys) {
                if (j[lb][k]) {
                  olderHistory = resolveOlder(j[lb][k]);
                  if (olderHistory) break;
                }
              }
              if (olderHistory) break;
      ...
      return {
        success: true,
        lastUpdate: String(found || ""),
        releaseNotes: String(releaseNotes || ""),
        olderHistory: String(olderHistory || ""),
        version,
      };
    } catch (err) {
      return { success: false, error: err.message || String(err) };
  ...
  async (_e, { installPath, folder }) => {
    try {
      ...
      const subdirs = [
        ...
        "ReleaseNotes",
        "releasenotes",
        path.join("SimObjects", "Airplanes"),
        path.join("SimObjects", "Rotorcraft"),
        path.join("SimObjects", "Aircraft"),
      ];
      const names = [
        "changelog.txt",
        "changelog.md",
      ...
      const isCandidate = (fn) =>
        names.includes(String(fn || "").toLowerCase());
      ...
      const kw =
        /(changelog|change\s*log|release\s*notes?|what'?s\s*new|update\s*history|version\s*\d|^v\d+\.\d+)/i;
      ...
      function scoreText(t) {
        ...
        for (const ln of lines) {
          ...
          if (
            /\b(202\d|20\d{2})\b/.test(ln) &&
            /(jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec|\d{1,2})/i.test(
              ln,
            )
          )
            s += 1;
...
ipcMain.handle("pkg:validate", async (_e, { installPath, folder }) => {
  try {
    ...
    if (!exists)
      return {
        success: false,
        error: "Resolved package directory not found",
        linkDir,
        realDir,
      };
    ...
    const hasManifest = await fs.promises
      ...
      .then((st) => st.isFile())
      .catch(() => false);
    const hasLayout = await fs.promises
      .stat(layoutPath)
      .then((st) => st.isFile())
      .catch(() => false);
      ...
      for (const e of ents) {
        if (e.isFile() && /^(aircraft)\.cfg$/i.test(e.name)) {
          ...
          try {
            ...
            let hasModelCfg = await fs.promises
              .stat(path.join(modelDir, "model.cfg"))
              .then((s) => s.isFile())
              .catch(() => false);
            let hasGltf = false;
            let gltfFiles = [];
            ...
            if (hasModelCfg) {
              try {
                const mtxt = await fs.promises.readFile(
                  path.join(modelDir, "model.cfg"),
                  "utf8",
                );
            ...
            try {
              const dents = await fs.promises.readdir(modelDir, {
                withFileTypes: true,
              });
